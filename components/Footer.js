
export default function Footer() {
    return (
        <div className="bg-gray-900 px-16 pt-16">
            <div className="grid grid-cols-3 justify-items-center">
                <div className="flex flex-col mr-[-400px] text-white">
                    <h3 className="text-3xl mb-3">Quick Links</h3>
                    <ul className="">
                        <li>World directories</li>
                        <li>State house</li>
                        <li>Ministries</li>
                        <li>Privacy policy</li>
                        <li>Testimonials</li>
                    </ul>
                </div>
                <div className="flex flex-col text-white">
                    <h3 className="text-3xl mb-3">Our sites</h3>
                    <ul>
                        <li>World directories</li>
                        <li>State house</li>
                        <li>Ministries</li>
                        <li>Privacy policy</li>
                        <li>Testimonials</li>
                    </ul>
                </div>
                <div className="flex flex-col text-white ml-[-400px]">
                    <h3 className="text-3xl mb-3">Reach us anytime</h3>
                    <span>Tel: +254 020 7800 701</span>
                    <div className="flex flex-row">
                        <svg className="w-7 h-7" xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" viewBox="0 0 64 64"><path d="M 31.820312 12 C 13.438312 12 12 13.439312 12 31.820312 L 12 32.179688 C 12 50.560688 13.438313 52 31.820312 52 L 32.179688 52 C 50.561688 52 52 50.560688 52 32.179688 L 52 32 C 52 13.452 50.548 12 32 12 L 31.820312 12 z M 43.994141 18 C 45.099141 17.997 45.997 18.889141 46 19.994141 C 46.003 21.099141 45.110859 21.997 44.005859 22 C 42.900859 22.003 42.003 21.110859 42 20.005859 C 41.997 18.900859 42.889141 18.003 43.994141 18 z M 31.976562 22 C 37.498562 21.987 41.987 26.454563 42 31.976562 C 42.013 37.498562 37.545437 41.987 32.023438 42 C 26.501437 42.013 22.013 37.545437 22 32.023438 C 21.987 26.501437 26.454563 22.013 31.976562 22 z M 31.986328 26 C 28.672328 26.008 25.992 28.701625 26 32.015625 C 26.008 35.328625 28.700672 38.008 32.013672 38 C 35.327672 37.992 38.008 35.299328 38 31.986328 C 37.992 28.672328 35.299328 25.992 31.986328 26 z"/></svg>
                        <svg className="w-7 h-7" xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" viewBox="0 0 64 64"><path d="M40.227,12C51.146,12,52,12.854,52,23.773v16.453C52,51.145,51.146,52,40.227,52h-2.089V37.438h5.134l0.807-5.77h-5.941 c0,0-0.009-3.378,0-4.221c0.017-1.649,1.394-2.483,2.577-2.471c1.183,0.012,3.634,0.004,3.634,0.004v-5.317 c0,0-2.123-0.275-4.349-0.298c-1.871-0.019-3.937,0.487-5.604,2.167c-1.696,1.709-1.963,4.254-1.996,7.364 c-0.009,0.9,0,2.772,0,2.772h-5.024v5.77H32.4V52h-8.626C12.854,52,12,51.145,12,40.227V23.773C12,12.854,12.854,12,23.773,12 H40.227z"/></svg>
                        <svg className="w-7 h-7" xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" viewBox="0 0 64 64"><path d="M57,17.114c-1.32,1.973-2.991,3.707-4.916,5.097c0.018,0.423,0.028,0.847,0.028,1.274c0,13.013-9.902,28.018-28.016,28.018 c-5.562,0-12.81-1.948-15.095-4.423c0.772,0.092,1.556,0.138,2.35,0.138c4.615,0,8.861-1.575,12.23-4.216 c-4.309-0.079-7.946-2.928-9.199-6.84c1.96,0.308,4.447-0.17,4.447-0.17s-7.7-1.322-7.899-9.779c2.226,1.291,4.46,1.231,4.46,1.231 s-4.441-2.734-4.379-8.195c0.037-3.221,1.331-4.953,1.331-4.953c8.414,10.361,20.298,10.29,20.298,10.29s-0.255-1.471-0.255-2.243 c0-5.437,4.408-9.847,9.847-9.847c2.832,0,5.391,1.196,7.187,3.111c2.245-0.443,4.353-1.263,6.255-2.391 c-0.859,3.44-4.329,5.448-4.329,5.448S54.314,18.335,57,17.114z"/></svg>
                        <svg className="w-7 h-7" xmlns="http://www.w3.org/2000/svg" fill="#FFFFFF" viewBox="0 0 64 64"><path d="M40.227,12C51.145,12,52,12.854,52,23.773v16.453C52,51.145,51.145,52,40.227,52H23.773C12.855,52,12,51.145,12,40.227 V23.773C12,12.854,12.855,12,23.773,12H40.227z M25.029,43V26.728h-5.057V43H25.029z M22.501,24.401 c1.625,0,2.947-1.322,2.947-2.949c0-1.625-1.322-2.947-2.947-2.947c-1.629,0-2.949,1.32-2.949,2.947S20.87,24.401,22.501,24.401z M44,43v-8.925c0-4.382-0.946-7.752-6.067-7.752c-2.46,0-4.109,1.349-4.785,2.628H33.08v-2.223h-4.851V43h5.054v-8.05 c0-2.122,0.405-4.178,3.036-4.178c2.594,0,2.628,2.427,2.628,4.315V43H44z"/></svg>
                    </div>
                </div>
            </div>
            <p className="text-white text-center mt-12 pb-8">Páginas Amarelas de Cabo Verde { new Date().getFullYear() } &copy; Todos os direitos reservados.</p>
        </div>
    );
}